...
Graph neural networks are justified by a _homophily_ assumption: nodes joined by an edge tend to
...
1. **The prior's value tracks the quality of the evidence, not label scarcity.** Our
   ...
   **refuted and reversed** (−7.07 accuracy points, 95% CI [−10.26, −3.91], _p_ = 6.6×10⁻⁴; and
   more sharply under macro-F1). A Markov random field _propagates_ label information; it does
...
_p_(y | X,G) ∝ exp[Σᵢ sᵢ(yᵢ) + β Σ₍ᵢ,ⱼ₎∈E **1**[yᵢ = yⱼ]], whose unary potentials are the
_frozen_ network's logits. Because inference consumes frozen logits, every MRF variant sees
